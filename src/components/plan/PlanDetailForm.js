    ...
    <div className="bg-white p-6 rounded-lg shadow-sm w-full mx-auto">
      ...
      <div className="grid grid-cols-2 gap-6 mb-8">
        <div>
          ...
          {renderInfoItem("Total Distance", `${formatNumber(plan.total_distance)} km`)}
          {renderInfoItem("Total Time", `${formatNumber(plan.total_time_serving)} minutes`)}
          {renderInfoItem("Total Demand", `${formatNumber(plan.total_demand)} kg`)}
          {renderInfoItem("Moving Cost", `${formatNumber(plan.moving_cost)} VND`)}
          {renderInfoItem("Labor Cost", `${formatNumber(plan.labor_cost)} VND`)}
          {renderInfoItem("Unloading Cost", `${formatNumber(plan.unloading_cost)} VND`)}
          {renderInfoItem("Order Value", `${formatNumber(plan.total_order_value)} VND`)}
          {renderInfoItem("Total Vehicles", plan.total_vehicle_used)}
          {renderInfoItem("Customers Served", plan.total_num_customer_served)}
        </div>
        <div>
          {renderInfoItem("Created at", new Date(plan.created_at).toLocaleString())}
          ...
          {renderInfoItem("Total Fee", `${formatNumber(plan.fee)} VND`)}
          {renderInfoItem("Total Plan Value", `${formatNumber(plan.total_plan_value)} VND`)}
          {renderInfoItem("Total Profit", `${formatNumber(plan.profit)} VND`)}
          {renderInfoItem("Total Routes", plan.total_routes)}
          {renderInfoItem("Customers Unserved", plan.total_num_customer_not_served)}
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-gray-100">
              {["ID", "Depot", "Demand (kg)", "Distance (km)", "Time (min)", "Fee (VND)", "Value (VND)", "Profit (VND)", "Status", "Action"].map((header) => (
          ...
          <tbody>
            {routes.map((route, index) => (
              <tr key={route.id} className={index % 2 === 0 ? "bg-gray-50" : "bg-white"}>
                ...
                <td className="p-2">{formatNumber(route.total_time_serving)}</td>
                ...
                <td className="p-2">
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                    route.is_served === 1 ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                  }`}>
