...
function AddOrderForm({ onClose, onOrderAdded }) {
    ...
    if (selectedProduct) {
      setSelectedProducts([
        ...selectedProducts,
        {
          ...
          originalQuantity: selectedProduct.quantity, // Lưu số lượng tồn kho ban đầu
          originalPrice: selectedProduct.price, // Lưu giá gốc
        },
      ]);
    }
  };
    ...
    setSelectedProducts(
      selectedProducts.map((product) => {
        if (product.id === productId) {
          // Chỉ giới hạn số lượng không vượt quá tồn kho, nhưng cho phép nhập bất kỳ số dương nào
  ...
  const handlePriceChange = (productId, price) => {
    setSelectedProducts(
      selectedProducts.map((product) => {
        if (product.id === productId) {
          // Cho phép nhập bất kỳ giá nào lớn hơn hoặc bằng 0
